...
This skill writes briefs in `planning/mockup-briefs/`. It does **not** generate mockups itself. The user feeds each brief into their tool of choice and saves the output to `docs/mockups/`. A future `generate-mockups` skill (planned around `nexu-io/open-design`) will consume these same briefs automatically.
...
|---|---|---|
| `planning/FEATURES.md` or `planning/features/` | **Yes** | Stop. *"Run `features` first — `mockups` needs per-feature User Flow, States, and Important sections to write concrete briefs."* |
| `docs/UX.md` | **Yes** | Stop. *"Run `ux-{web,app,ios}` first — `mockups` needs the screen inventory and component names."* |
| `docs/DESIGN.md` | **Yes** | Stop. *"Run `design` first — `mockups` needs the brand spec to anchor visual references."* |

No fallback. Mockup quality is bounded by upstream artifact quality; this skill refuses to paper over missing planning.

## What gets generated

`planning/mockup-briefs/` directory:

```
planning/mockup-briefs/
...
For each anchor screen, write a brief at `planning/mockup-briefs/01-anchor-<screen>.md`. Use the Brief Format below.

Pass 1 input pulls:
- Full `docs/DESIGN.md` (the entire brand spec — this is where DESIGN.md pays off most)
- `docs/UX.md § <Anchor Section>` only (one section)
...
> *"Generated Pass 1 anchor brief at `planning/mockup-briefs/01-anchor-<screen>.md`. Feed this into Claude Design / Stitch / Open Design / v0. Iterate until the visual language is locked. When you have a final anchor mockup, save it to `docs/mockups/anchor-<screen>.{png,jsx,html}` and tell me to continue. I'll then generate Pass 2 briefs that reference the locked anchor."*
...
1. Verify `docs/mockups/anchor-<screen>.{png,jsx,html}` exists.
2. Identify high-stakes screens from `docs/UX.md` (every page/screen with its own section that isn't an anchor).
3. For each, write a brief at `planning/mockup-briefs/02-<screen>.md`.

Pass 2 input pulls per screen:
- Reference to the locked anchor mockup (path + brief description of what was locked)
- `docs/UX.md § <Screen Section>` (one section)
...
For each screen flagged with meaningful states in FEATURES § States, write a state variant brief at `planning/mockup-briefs/03-<screen>-<state>.md`.
...
Write `planning/mockup-briefs/README.md`:
...
1. **Pass 1** — run anchor brief(s) first. Iterate heavily. Save locked mockup(s) to `docs/mockups/anchor-*.{png,jsx,html}`.
...
|---|---|---|---|---|
| `01-anchor-<screen>.md` | 1 | <screen> | draft / ready / used | `docs/mockups/anchor-<screen>.{ext}` |
| `02-<screen>.md` | 2 | <screen> | draft / ready / used | `docs/mockups/<screen>.{ext}` |
| ... | ... | ... | ... | ... |
...
output_path: docs/mockups/<screen>.{png,jsx,html}
...
- **Output target:** `docs/mockups/<screen>.{png,jsx,html}`
...
- See `docs/DESIGN.md` for the full brand spec.
...
- [Pass 2: `docs/mockups/anchor-<screen>.{png,jsx,html}` — match this visual language exactly]
- [Pass 3: `docs/mockups/<screen>.{png,jsx,html}` — variant of this screen]
...
- **Special note:** Stitch accepts a DESIGN.md file directly. Attach `docs/DESIGN.md` to the conversation, then the brief reads as a focused per-screen prompt.
...
- **Output:** JSX/TSX with Tailwind classes. The output drops directly into `docs/mockups/<screen>.jsx`.
