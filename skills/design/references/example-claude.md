# Reference Example: Claude DESIGN.md

This is a reference DESIGN.md written in the [awesome-design-md](https://github.com/VoltAgent/awesome-design-md) format (MIT licensed, © VoltAgent), capturing Anthropic's Claude website. Use it as the North Star for voice, structure, and depth when writing your own DESIGN.md outputs. The official Claude template lives in the upstream catalog at `unpkg.com/getdesign/templates/claude.md`.

What to notice:

- **Section 1 is prose, not bullets** — three paragraphs of brand storytelling before any bullet list
- **Every color has an evocative name** — "Parchment," "Terracotta Brand," "Olive Gray" — not `--color-1`
- **Each color has 1-2 sentences of identity** — what it is *and* what role/character it plays
- **Multiple named button variants** — "Warm Sand," "Brand Terracotta," "Dark Charcoal" — not just "Primary/Secondary"
- **Section 4 has a "Distinctive Components" subsection** for site-unique patterns (Model Comparison Cards, Organic Illustrations, Light/Dark Section Alternation)
- **Section 6 has a "Shadow Philosophy" prose paragraph** explaining the *theory* behind ring-based depth
- **Border radius is a named scale** ("Sharp," "Subtly rounded," "Comfortably rounded," ... "Maximum rounded")
- **Do's/Don'ts are brand-specific** — "Don't use cool blue-grays anywhere" is unique to Claude; "Use consistent spacing" would be useless boilerplate
